...
import { getAllUser, createUser, loginUser, deleteUser, updateUser, getUserById } from './useroperation.js';
...
app.get('/api/user/:id', async (req, res) => {
  try {
    ...
    const modifiedCount = await getUserById(id);
   res.status(200).json({ message: 'User found successfully', user: modifiedCount });
  } catch (error) {
    console.error('Update error:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
